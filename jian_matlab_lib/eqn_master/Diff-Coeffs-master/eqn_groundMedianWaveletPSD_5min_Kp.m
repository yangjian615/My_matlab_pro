function outputMatrix = eqn_groundMedianWaveletPSD_5min_Kp(day, FREQS, ...
...
if ~isempty(data_init)
    ...
    nFreqs = length(FREQS);
    outputMatrix = nan(288, nFreqs + 1 + 1 + 1); % nFreqs + Kp + MLT + time

    % do the processing for each minute (UT)
    for i=0:287
        timeInd = find(t >= t_beg + i/288 & t < t_beg + (i+1)/288);
        timeInterval = t(timeInd);

        % Accept intervals with more than 50% real data
        if ((length(timeInterval) - sum(isnan(B(timeInd))))*SAMPLING_TIME > 60*0.5)
